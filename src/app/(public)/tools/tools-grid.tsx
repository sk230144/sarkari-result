...
export function ToolsGrid({ isPremium }: { isPremium: boolean }) {
  return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {tools.map((tool) => {
            ...
            return (
              <Card
                key={tool.href}
                className={`card-3d relative overflow-hidden ${locked ? "ring-1 ring-violet-200" : ""}`}
              >
                {tool.premium && (
                  <div className="absolute top-3 right-3 z-10">
                    <div className={`flex items-center gap-1 rounded-full px-2.5 py-1 shadow-md ${
                      isPremium
                ...
                <CardHeader>
                  <div className="flex items-center gap-3">
                    <div
                      className={`icon-3d h-10 w-10 rounded-xl flex items-center justify-center ${locked ? "bg-violet-50" : "bg-white"}`}
                    >
                      <Icon
                        className={`h-5 w-5 ${locked ? "text-violet-500" : "text-slate-600"}`}
                      />
                    </div>
                    <div className="pr-20">
